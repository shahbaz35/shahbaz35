...
def getCardDetails(x, y):
    ...
    try:
        ...
        if accountID == "0":
            ...
            # print(accountID)
            auto_emailAddress = email
            print(auto_emailAddress)
        ...
        try :
            ...
            auto_cardNumber = cardDetails[1]
            print("16 Digits: " + auto_cardNumber)
            auto_expiryMonth = cardDetails[2]
            print("Expiry 1: " + auto_expiryMonth)
            auto_expiryYear = cardDetails[3]
            print("Expiry 2: " + auto_expiryYear)
            auto_cvvNumber = cardDetails[4]
            print("CVV: " + auto_cvvNumber)
            auto_nameOnCard = cardDetails[5]
            print("CardHolder Name: "+auto_nameOnCard)
        except IndexError:
            pass
...
def page2():
    ...
    regLabel = Label(root, text=" Enter Car Registration Without Spaces: ", padx = 50, pady = 10, bg= "Light Blue").grid(row=4, column=0)
    regTextBox = Entry(root, width=50, borderwidth= 4)
    regTextBox.grid(row= 5,column=0, padx= 50, pady= 10)

    submitButton = Button(root, text=" Submit ",width= 15,  command= lambda : submission(pcnTextBox.get(), regTextBox.get()), fg = "black", bg = "light pink")
...
def submission(PCN,reg):
    ...
    if auto_refNo != '' and auto_numberPlate != '':
        pcnFailLabel.grid_remove()
        regFailLabel.grid_remove()

        automation(auto_refNo, auto_numberPlate)
...
def errorChecker(x):
    ...
    if error == '':
        ...
        try :
            ...
            pass
        finalMessageLabel = Label(root, text= error, bg = "Light Blue")
        finalMessageLabel.grid(row = 10,column = 0)
        # driver.quit()
        #raise SystemExit(0)
